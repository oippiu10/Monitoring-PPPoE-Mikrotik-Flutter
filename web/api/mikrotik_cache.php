...
class MikrotikCache {
    ...
    public function get(string $key) {
        ...
        if ($u) { $u->bind_param("s", $key); $u->execute(); $u->close(); }
    ...
    public function set(string $key, $data, int $ttl = 60): void {
        ...
        $stmt = $this->conn->prepare(
            "INSERT INTO mikrotik_cache (cache_key, data, expires_at, hit_count)
             VALUES (?, ?, ?, 0)
             ON DUPLICATE KEY UPDATE data = VALUES(data), expires_at = VALUES(expires_at), hit_count = 0"
        );
        if (!$stmt) return;
     ...
     *   });
     */
    public function getOrFetch(string $key, int $ttl, callable $fetcher) {
        $cached = $this->get($key);
        if ($cached !== null) {
            return ['data' => $cached, 'from_cache' => true, 'ttl' => $ttl];
        }

        // Cache miss → fetch dari Mikrotik
        try {
            $fresh = $fetcher();
