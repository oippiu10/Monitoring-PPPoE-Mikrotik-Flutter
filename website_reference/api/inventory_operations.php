...
function ensure_inventory_tables(mysqli $conn) {
    ...
    if (!in_array('router_id', $cols))   $conn->query("ALTER TABLE inventory ADD COLUMN router_id INT NOT NULL DEFAULT 0 AFTER id");
    ...
    $cols_mov = [];
    $res_mov = $conn->query("SHOW COLUMNS FROM inventory_movements");
    if ($res_mov) { while ($r = $res_mov->fetch_assoc()) $cols_mov[] = $r['Field']; }
    if (!in_array('router_id', $cols_mov)) $conn->query("ALTER TABLE inventory_movements ADD COLUMN router_id INT NOT NULL DEFAULT 0 AFTER id");
}
...
switch ($action) {
    ...
    case 'edit':
        require_admin_role(['admin', 'administrator', 'super_admin', 'super admin', 'superadministrator'], 'Hanya admin yang boleh mengubah inventory.');
        $data = input_json();
        $id = intval($data['id'] ?? 0);
        $name = trim($data['name'] ?? '');
        if ($name === '') throw new Exception('Nama barang wajib diisi');
        ...
        $stmt = $conn->prepare("UPDATE inventory SET name=?, category=?, unit=?, price=?, description=? WHERE id=? AND router_id=?");
        $stmt->bind_param("sssdsii", $name, $category, $unit, $price, $description, $id, $router_id);
        $ok = $stmt->execute();
        if ($ok) log_admin_activity($conn, 'inventory_edit', 'Edit item inventory: ' . $name, (int)($_SESSION['admin_id'] ?? 0));
        echo json_encode(['success' => $ok, 'message' => $ok ? 'Item berhasil diubah' : $conn->error]);
        break;
    ...
    case 'log':
        ...
        $stmt = $conn->prepare("
            ...
            WHERE m.router_id = ? AND m.inventory_id = ?
            ORDER BY m.created_at DESC
        ");
        $stmt->bind_param("ii", $router_id, $inventory_id);
        $stmt->execute();
        $res = $stmt->get_result();
        $logs = [];
        while ($row = $res->fetch_assoc()) {
