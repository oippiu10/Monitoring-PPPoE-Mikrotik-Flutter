...
$genSql = "INSERT IGNORE INTO invoices (user_id, router_id, month, year, amount, status)
           ...
           WHERE u.router_id = ? 
             AND (YEAR(IFNULL(u.created_at, '2026-05-01')) < ? 
                  OR (YEAR(IFNULL(u.created_at, '2026-05-01')) = ? AND MONTH(IFNULL(u.created_at, '2026-05-01')) <= ?))";
...
if (!$genStmt) {
    ...
    $genStmt->bind_param("iisiii", $month, $year, $router_id, $year, $year, $month);
    $genStmt->execute();
    $genStmt->close();
}
...
// Hanya tampilkan pelanggan yang sudah bergabung pada bulan/tahun yang sedang dilihat
$where[] = "(YEAR(IFNULL(u.created_at, '2026-05-01')) < ? OR (YEAR(IFNULL(u.created_at, '2026-05-01')) = ? AND MONTH(IFNULL(u.created_at, '2026-05-01')) <= ?))";
$params[] = $year;
$params[] = $year;
$params[] = $month;
$types .= "iii";
...
if ($status === 'paid') {
    $where[] = "p.id IS NOT NULL AND p.method != 'titipan'";
} elseif ($status === 'titipan') {
    $where[] = "p.id IS NOT NULL AND p.method = 'titipan'";
...
$sumSQL = "SELECT 
             SUM(CASE WHEN p.id IS NOT NULL AND p.method != 'titipan' THEN 1 ELSE 0 END) as total_paid,
             ...
             SUM(CASE WHEN p.method = 'titipan' THEN IFNULL(p.amount, 0) ELSE 0 END) as titipan,
             SUM(CASE WHEN p.method = 'titipan' THEN 1 ELSE 0 END) as titipan_users,
...
$summary = [
    ...
    'titipan' => (float)($sumResult['titipan'] ?? 0),
    'titipan_users' => (int)($sumResult['titipan_users'] ?? 0),
