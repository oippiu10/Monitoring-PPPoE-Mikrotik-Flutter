...
$rtSoftwareId = $routerData['software_id'] ?? strval($router_id);
...
try {
    switch ($action) {
        ...
        case 'secret_edit':
            ...
            // Auto-sync ke tabel users jika username sama
            $newProfile = $params['profile'] ?? 'default';
            if (!empty($params['password'])) {
                $newPassword = $params['password'];
                $stmtSync = $conn->prepare("UPDATE users SET profile = ?, password = ? WHERE username = ? AND (router_id = ? OR router_id = ?)");
                $stmtSync->bind_param("sssss", $newProfile, $newPassword, $params['name'], $rtSoftwareId, $router_id);
            } else {
                $stmtSync = $conn->prepare("UPDATE users SET profile = ? WHERE username = ? AND (router_id = ? OR router_id = ?)");
                $stmtSync->bind_param("ssss", $newProfile, $params['name'], $rtSoftwareId, $router_id);
            }
            $stmtSync->execute();
            $stmtSync->close();
