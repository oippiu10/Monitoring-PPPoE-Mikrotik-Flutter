...
try {
    ...
    if ($action === 'edit_extra') {
        ...
        if ($redaman !== '') {
            $numericPart = preg_replace('/[^-0-9.]/', '', $redaman);
        ...
        $odpPort = ($input['odp_port'] ?? '') !== '' && $input['odp_port'] !== null ? intval($input['odp_port']) : null;
        ...
        $stmt = $conn->prepare("UPDATE users SET wa=?, alamat=?, maps=?, redaman=?, odp_id=?, odp_port=?, tanggal_tagihan=?, tanggal_dibuat=?, lat=?, lng=?, tipe_langganan=?, updated_at=NOW() WHERE username=? AND router_id=?");
        if (!$stmt) throw new Exception($conn->error);
        $stmt->bind_param('ssssiiisddsss', $wa, $alamat, $maps, $redaman, $odpId, $odpPort, $tanggalTagihan, $tanggalDibuat, $lat, $lng, $tipeLangganan, $username, $routerId);
        $stmt->execute();
    ...
    if ($action === 'add') {
        ...
        $tipeLangganan = $input['tipe_langganan'] ?? 'pascabayar';
        $odpPort = ($input['odp_port'] ?? '') !== '' && $input['odp_port'] !== null ? intval($input['odp_port']) : null;
        $stmt = $conn->prepare("INSERT INTO users (router_id, username, password, profile, wa, alamat, maps, redaman, odp_id, odp_port, tanggal_tagihan, tanggal_dibuat, on_router, lat, lng, tipe_langganan) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 1, ?, ?, ?)");
        $stmt->bind_param('ssssssssiiisdds', $routerId, $username, $password, $profile, $wa, $alamat, $maps, $redaman, $odpId, $odpPort, $tanggalTagihan, $tanggalDibuat, $lat, $lng, $tipeLangganan);
        $stmt->execute();
        $stmt->close();
        ...
        $odpPort = ($input['odp_port'] ?? '') !== '' && $input['odp_port'] !== null ? intval($input['odp_port']) : null;
        $sets[] = 'odp_port=?'; $vals[] = $odpPort; $types .= 'i';
