...
$stmtUser = $conn->prepare("
    SELECT u.username, u.alamat, u.profile, u.created_at, pr.price as current_price
    FROM users u
...
// Tentukan kapan pelanggan mulai ditagih berdasarkan tanggal instalasi/dibuat
$user_created = strtotime($user['created_at'] ?? '2026-05-01');
$u_start_year = intval(date('Y', $user_created));
$u_start_month = intval(date('n', $user_created));

// Tweak khusus: Batas paling awal sistem berjalan adalah Mei 2026
if ($u_start_year < 2026 || ($u_start_year == 2026 && $u_start_month < 5)) {
    $u_start_year = 2026;
    $u_start_month = 5;
}
...
for ($m = 1; $m <= 12; $m++) {
    ...
    $is_before_start = ($year < $u_start_year) || ($year == $u_start_year && $m < $u_start_month);
    ...
    if ($is_future) {
        ...
        // Jangan tagih untuk bulan-bulan sebelum pelanggan berlangganan
        $iuran_bulanan = 0;
        $bayar = 0;
        $carry_over = 0;
        $tagihan_total = 0;
