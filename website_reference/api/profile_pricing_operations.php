...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && $operation === 'add') {
    ...
    if ($stmt->execute()) {
        ...
        session_start();
        log_admin_activity($conn, 'pricing_create', "Set harga profil {$profile_name}: Rp " . number_format($price, 0, ',', '.'), (int)($_SESSION['admin_id'] ?? 0));
...
if ($_SERVER['REQUEST_METHOD'] === 'PUT' || ($_SERVER['REQUEST_METHOD'] === 'POST' && $operation === 'update')) {
    ...
    if ($stmt->execute()) {
        ...
        session_start();
        $tgt = isset($data['id']) ? "ID " . $data['id'] : $data['profile_name'];
        log_admin_activity($conn, 'pricing_update', "Update harga profil {$tgt}", (int)($_SESSION['admin_id'] ?? 0));
...
if ($_SERVER['REQUEST_METHOD'] === 'DELETE' || ($_SERVER['REQUEST_METHOD'] === 'POST' && $operation === 'delete')) {
    ...
    if ($stmt->execute()) {
        ...
        session_start();
        $tgt = isset($data['id']) ? "ID " . $data['id'] : $data['profile_name'];
        log_admin_activity($conn, 'pricing_delete', "Hapus harga profil {$tgt}", (int)($_SESSION['admin_id'] ?? 0));
