...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && $operation === 'add') {
    ...
    if ($stmt->execute()) {
        session_start();
        log_admin_activity($conn, 'odp_create', "Tambah ODP: {$data['name']}", (int)($_SESSION['admin_id'] ?? 0));
        echo json_encode([
            "success" => true,
...
// Handle POST request for update operation
    ...
    if ($stmt->execute()) {
        if ($stmt->affected_rows > 0) {
            session_start();
            log_admin_activity($conn, 'odp_update', "Update ODP: {$data['name']}", (int)($_SESSION['admin_id'] ?? 0));
            echo json_encode([
                "success" => true,
...
// Handle POST request for delete operation
    ...
    if ($stmt->execute()) {
        if ($stmt->affected_rows > 0) {
            session_start();
            log_admin_activity($conn, 'odp_delete', "Hapus ODP ID: {$data['id']}", (int)($_SESSION['admin_id'] ?? 0));
            echo json_encode([
                "success" => true,
