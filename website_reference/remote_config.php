...
if ($resOdcCols) {
    ...
    try {
        if (!in_array('cable_color_id', $odcCols)) {
            $conn->query("ALTER TABLE odc ADD COLUMN `cable_color_id` INT DEFAULT NULL");
        }
        if (!in_array('core_number', $odcCols)) {
            $conn->query("ALTER TABLE odc ADD COLUMN `core_number` INT DEFAULT NULL");
        }
    } catch (Exception $e) { }
}
...
if (isset($odpCols)) {
    try {
        if (!in_array('cable_color_id', $odpCols)) {
            $conn->query("ALTER TABLE odp ADD COLUMN `cable_color_id` INT DEFAULT NULL");
        }
        if (!in_array('core_number', $odpCols)) {
            $conn->query("ALTER TABLE odp ADD COLUMN `core_number` INT DEFAULT NULL");
        }
    } catch (Exception $e) { }
}
