...
export function Dashboard() {
  ...
  const handleExportCSV = () => {
    ...
    csvContent += "=== LAPORAN DASHBOARD ISP ===\n\n"
    csvContent += `Tanggal Export:,${new Date().toLocaleDateString('id-ID')}\n`
    csvContent += `Router Aktif:,${activeRouter?.name || 'Unknown'}\n\n`
    
    csvContent += "--- Ringkasan Utama ---\n"
    csvContent += `Total Pelanggan Terdaftar:,${stats.total_users || 0}\n`
    csvContent += `Sesi PPPoE Aktif:,${stats.online_users || 0}\n`
    csvContent += `Total Secret MikroTik:,${stats.total_secrets || 0}\n`
    csvContent += `Pendapatan Bulan Ini:,Rp ${stats.revenue || 0}\n`
    csvContent += `Tagihan Belum Lunas:,${stats.pending_payments || 0}\n\n`
    
    csvContent += "--- Distribusi Paket Profil ---\n"
    csvContent += "Nama Profil,Jumlah Pengguna\n"
    if (stats.profile_distribution) {
      Object.entries(stats.profile_distribution).forEach(([profile, count]) => {
    ...
    link.setAttribute("download", `Laporan_Dashboard_${new Date().toISOString().split('T')[0]}.csv`)
      ...
      <Main fluid>
        ...
        <div className='space-y-4'>
            ...
            <div className='grid grid-cols-1 gap-4 lg:grid-cols-7'>
              <Card className='col-span-1 lg:col-span-4 shadow-xl border-none bg-card/50 backdrop-blur-sm overflow-hidden flex flex-col'>
                ...
                <CardContent className='ps-2 flex-1 pb-0 min-h-[350px] relative'>
