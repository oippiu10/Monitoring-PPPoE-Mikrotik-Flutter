...
import { Info, MapPin, Check, ChevronsUpDown } from 'lucide-react'
...
export function CustomerMutateDialog({ isOpen, onClose, customer, profiles, odps }: Props) {
  ...
  const selectedOdpId = form.watch('odp_id')

  const selectedOdp = useMemo(() => {
    if (!selectedOdpId || selectedOdpId === 'none') return null
    return odps.find(o => String(o.id) === String(selectedOdpId))
  }, [selectedOdpId, odps])

  const portOptions = useMemo(() => {
    if (!selectedOdp) return []
    let cap = 0
    if (selectedOdp.type === 'splitter') {
      const parts = selectedOdp.splitter_type?.split(':')
      cap = parts && parts.length > 1 ? parseInt(parts[1]) : 0
    } else {
      cap = selectedOdp.ratio_total || 0
    }
          ...
          <div className="p-4">
            ...
            <form id='customer-form' onSubmit={form.handleSubmit(onSubmit)} className='space-y-3'>
                ...
                <div className='grid grid-cols-2 gap-3'>
                    ...
                    name='odp_id'
                    render={({ field }) => (
                        <FormItem className="space-y-1 flex flex-col pt-[5px]">
                        ...
                        <Popover>
                            ...
                            <FormControl>
                                ...
                                {field.value
                                    ? odps.find(
                                        (o) => o.id.toString() === field.value?.toString()
                                    )?.name || "Tanpa ODP"
                                    : "Tanpa ODP"}
                            ...
                            <Command>
                                <CommandInput placeholder="Cari ODP..." className="text-xs" />
                                <CommandList>
                                <CommandEmpty className="text-xs py-4 text-center">ODP tidak ditemukan.</CommandEmpty>
                                <CommandGroup>
                                    <CommandItem
                                        value=" Tanpa ODP"
                                        className="text-xs"
                                    ...
                                    >
                                        <Check
                                            className={cn(
                                            "mr-2 h-4 w-4",
                                            !field.value || field.value.toString() === 'none' ? "opacity-100" : "opacity-0"
                                            )}
                                        />
                                        Tanpa ODP
                                    </CommandItem>
                                    ...
                                    <CommandItem
                                        ...
                                        className="text-xs"
                                        onSelect={() => {
                                            field.onChange(parseInt(o.id.toString()))
                                            form.setValue('odp_port', '')
                                            document.body.click()
                                        }}
                                    >
                                        ...
                                        className={cn(
                                            "mr-2 h-4 w-4",
                                            field.value?.toString() === o.id.toString()
                                            ? "opacity-100"
                                            : "opacity-0"
                                        )}
                                        />
                    ...
                    {selectedOdp && (
                        ...
                        render={({ field }) => (
                            <FormItem className="space-y-1">
                            <FormLabel className="text-xs">Port ODP</FormLabel>
                            <Select 
                                onValueChange={(val) => field.onChange(val === 'none' ? '' : parseInt(val))} 
                            ...
                            >
                                ...
                                <SelectContent>
                                    <SelectItem value='none' className="text-xs">Pilih Port...</SelectItem>
                                    {portOptions.map(p => (
                                        <SelectItem key={p} value={p.toString()} className="text-xs">Port {p}</SelectItem>
                                    ))}
                                </SelectContent>
                            </Select>
                            <FormMessage className="text-[10px]" />
