  ...
  const columns: ColumnDef<PPPActiveConnection>[] = [
    ...
    },
    {
      ...
      cell: ({ row }) => {
        ...
        if (!address || address === '-') return <span className="font-mono text-sm text-muted-foreground">-</span>;
        return (
          <a 
            href={`http://${address}`}
            target="_blank"
            rel="noopener noreferrer"
            className="font-mono text-sm text-primary hover:underline hover:text-primary/80 inline-block px-1.5 py-0.5 rounded bg-primary/10 transition-colors"
