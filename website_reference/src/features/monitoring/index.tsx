...
import { Cpu, HardDrive, Zap, Clock, Activity, Network, MousePointerClick, Filter, AlertTriangle } from 'lucide-react'
import {
  DropdownMenu,
...
export function Monitoring() {
  ...
  const { data: resource, isError: isResourceError, error: resourceError } = useQuery<RouterResource | null>({
    ...
    queryFn: async () => {
      ...
      if (res.data && res.data.success === false) {
        throw new Error(res.data.message || 'Gagal terhubung ke API MikroTik')
      }
  ...
  const { data: interfaces, isError: isInterfacesError, error: interfacesError } = useQuery<InterfaceData[]>({
  ...
  const { data: identity } = useQuery<{ name: string } | null>({
    ...
    enabled: !!activeRouter && !isInterfacesError && !isResourceError,
    refetchInterval: 60000, // 1 minute
  })

  // Consolidate connection errors
  const connectionError = useMemo(() => {
    if (isResourceError) return (resourceError as any)?.response?.data?.message || resourceError?.message || 'Koneksi ke MikroTik gagal (Resource)'
    if (isInterfacesError) return (interfacesError as any)?.response?.data?.message || interfacesError?.message || 'Koneksi ke MikroTik gagal (Interfaces)'
    return null
  }, [isResourceError, isInterfacesError, resourceError, interfacesError])
    ...
    <>
      ...
      <Main className='space-y-6' fluid>
        {connectionError && (
          <Alert variant='destructive' className='border-red-500 bg-red-500/10 text-red-600 dark:text-red-400'>
            ...
            <AlertDescription className='text-xs font-semibold'>
              {connectionError}. Silakan periksa koneksi VPN router atau pastikan kredensial IP/Port API MikroTik Anda sudah benar.
            </AlertDescription>
          </Alert>
        )}
        ...
        <div className='grid gap-4 md:grid-cols-2 lg:grid-cols-4'>
          ...
          <Card className="relative overflow-hidden border-none shadow-lg bg-linear-to-br from-emerald-500 to-emerald-600 text-white">
            <div className="absolute top-0 right-0 p-3 opacity-20">
          ...
          <Card className="relative overflow-hidden border-none shadow-lg bg-linear-to-br from-orange-500 to-orange-600 text-white">
            <div className="absolute top-0 right-0 p-3 opacity-20">
                ...
                <Card className="shadow-xl border-none bg-card/50 backdrop-blur-sm overflow-hidden h-full">
                  ...
                  <CardContent className="pt-6 space-y-4">
                       ...
                       <div className="flex justify-between items-center pt-2">
                            ...
                            {connectionError ? (
                                <Badge variant="destructive" className="bg-red-500 h-5 px-3 hover:bg-red-600 font-bold border-0 text-[10px] uppercase">Disconnected</Badge>
                            ) : (
