...
import { columns as baseColumns } from './odp-columns'
...
export function ODPTable({ 
  ...
  // Fuzzy filter: normalize spasi & huruf besar supaya 'ODP1' cocok dengan 'ODP 1'
  const fuzzyFilter: FilterFn<ODP> = (row, _columnId, filterValue: string) => {
    const normalize = (s: string) => s.toLowerCase().replace(/\s+/g, '')
    const query = normalize(filterValue)
    const fields = [
      row.original.name,
      row.original.location,
      row.original.odc_name,
      row.original.cable_color_name,
      row.original.splitter_type,
    ]
  ...
  // Kolom checkbox untuk seleksi baris
  const selectColumn = {
    id: 'select',
    header: ({ table }: any) => (
      <Checkbox
