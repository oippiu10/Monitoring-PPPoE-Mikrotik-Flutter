...
export function FinanceDashboard() {
  ...
  const { data: recentActivities } = useQuery({
    queryKey: ['finance-recent-activities', routerId],
    queryFn: async () => {
      const res = await api.get('/finance_report.php', {
        params: { action: 'recent_activities', router_id: routerId, limit: 8 }
      })
      return res.data.data || []
    },
    enabled: !!routerId,
  ...
  return (
    <>
      ...
      <Main className='space-y-6' fluid>
        ...
        {/* Tren Pendapatan & Pengeluaran 12 Bulan - Lebar Penuh */}
        <Card className='border-none shadow-lg w-full'>
          <CardHeader className='pb-2'>
            <CardTitle className='text-sm font-black uppercase tracking-widest flex items-center gap-2'>
              <TrendingUp className='h-4 w-4 text-primary' /> Arus Kas {selYear}
            </CardTitle>
          </CardHeader>
          <CardContent>
            <ResponsiveContainer width='100%' height={250}>
              <BarChart data={annual?.data || []} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
                ...
                <Tooltip
                  ...
                  formatter={(v: any, name: any) => [privacyMode ? '••••••' : fmt(Number(v)), name === 'revenue' ? 'Pendapatan' : 'Pengeluaran']}
                  labelFormatter={l => `Bulan: ${MONTHS[(l as number) - 1] || l}`}
                  contentStyle={{ backgroundColor: 'hsl(var(--popover))', color: 'hsl(var(--popover-foreground))', borderRadius: '12px', border: '1px solid hsl(var(--border))', boxShadow: '0 10px 25px -5px rgba(0, 0, 0, 0.1), 0 8px 10px -6px rgba(0, 0, 0, 0.1)', fontSize: 12, fontWeight: 800 }}
                />
                <Legend wrapperStyle={{ fontSize: '10px', fontWeight: 'bold' }} />
                <Bar dataKey='revenue' name="Pendapatan" fill='url(#colorRevenue)' radius={[6, 6, 0, 0]} maxBarSize={40} />
                <Bar dataKey='expense' name="Pengeluaran" fill='url(#colorExpense)' radius={[6, 6, 0, 0]} maxBarSize={40} />
              </BarChart>
            </ResponsiveContainer>
          </CardContent>
        </Card>
      ...
      <div className='mt-6 grid gap-6 lg:grid-cols-1'>
        ...
        <Card className='border-none shadow-lg flex flex-col'>
          <CardHeader className='pb-2 flex flex-row items-center justify-between border-b border-border/50 bg-muted/10'>
            <CardTitle className='text-sm font-black uppercase tracking-widest flex items-center gap-2'>
              <Clock className='h-4 w-4 text-primary' /> Log Aktivitas Keuangan
            </CardTitle>
          </CardHeader>
          <CardContent className='p-0 flex-1 overflow-auto max-h-[300px]'>
            <div className='divide-y divide-border/50'>
              {(recentActivities || []).length === 0 && (
                <p className='text-center text-muted-foreground text-sm py-8'>Belum ada aktivitas</p>
              )}
              {(recentActivities || []).map((a: any) => {
                const isDelete = a.action.includes('unpaid')
                const isEdit = a.action.includes('sync')
                const isAdd = a.action.includes('paid') && !isDelete
                return (
                  <div key={a.id} className='px-6 py-3 hover:bg-muted/30 transition-colors flex items-start gap-3'>
                    <div className={cn('mt-0.5 h-8 w-8 shrink-0 rounded-full flex items-center justify-center', 
                      isDelete ? 'bg-red-100 dark:bg-red-900/30' : 
                      isEdit ? 'bg-blue-100 dark:bg-blue-900/30' : 
                      'bg-green-100 dark:bg-green-900/30'
                    )}>
                      {isDelete ? <Trash2 className='h-3.5 w-3.5 text-red-600' /> : 
                       isEdit ? <SettingsIcon className='h-3.5 w-3.5 text-blue-600' /> : 
                       <CheckCircle2 className='h-3.5 w-3.5 text-green-600' />}
                    </div>
                    <div>
                      <p className='text-xs font-medium text-foreground leading-relaxed'>
                        {a.description.replace(/^\[R:[^\]]+\]\s*/, '')}
                      </p>
                      <p className='text-[10px] text-muted-foreground mt-1'>
                        {a.created_at} &bull; Oleh: <span className="font-semibold">{a.admin_name || 'Sistem'}</span>
                      </p>
                    </div>
                  </div>
                )
