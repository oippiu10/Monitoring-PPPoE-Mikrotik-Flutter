...
export function FinanceBilling() {
  ...
  // Fetch current web/ISP settings
  ...
  const generateInvoices = useMutation({
    mutationFn: async () => {
      const res = await api.post('/generate_invoices.php', {
        router_id: activeRouter?.software_id || activeRouter?.id,
        month,
        year
      })
    ...
    onSuccess: (d) => {
      if (d.success) {
        toast.success(d.message || `Berhasil menerbitkan ${d.inserted || 0} tagihan baru.`)
        refetch()
      } else toast.error(d.message || 'Gagal menerbitkan tagihan')
    },
    onError: () => {
  ...
  const markPaid = useMutation({
    mutationFn: async (row: any) => {
      const res = await api.post('/payment_operations.php', {
        ...
        is_discount: row.isDiscount !== undefined ? row.isDiscount : false,
        month,
        year,
      })
      return res.data
  ...
  const syncInvoice = useMutation({
    ...
    onSuccess: (d) => {
      if (d.success) {
        toast.success('Tagihan berhasil disesuaikan!')
        queryClient.invalidateQueries({ queryKey: ['billing'] })
        queryClient.invalidateQueries({ queryKey: ['receivable'] })
        queryClient.invalidateQueries({ queryKey: ['finance-kpi'] })
        queryClient.invalidateQueries({ queryKey: ['finance-annual'] })
      } else toast.error(d.message || 'Gagal menyesuaikan tagihan')
    },
  })
  ...
  const syncInvoiceMutate = syncInvoice.mutate
  ...
  const columns = useMemo(() => getBillingColumns({
    ...
    syncInvoice: syncInvoiceMutate,
    month,
    year,
    settings,
    dataLength: tableData.length,
    fmt
  }), [permissions, selectedRows, toggleSelectRow, toggleSelectAll, setHistoryUser, setPaymentCardUser, setPaidDialog, handleWA, confirmAction, markUnpaidMutate, syncInvoiceMutate, month, year, settings, tableData.length])
    ...
    <>
      ...
      <Main className='space-y-4' fluid>
        ...
        <div className='grid grid-cols-2 gap-4 lg:grid-cols-5'>
          ...
          <Card className='relative overflow-hidden border-none shadow-lg bg-linear-to-br from-violet-500 to-purple-600 text-white'>
            <CardHeader className='pb-2'>
              <CardTitle className='text-[10px] font-black uppercase tracking-widest opacity-80'>Titipan</CardTitle>
            </CardHeader>
            <CardContent>
              <div className='text-xl xl:text-2xl font-black mb-1 truncate'>
                <PrivacyText>{fmt(summary.titipan || 0)}</PrivacyText>
              </div>
              <div className='flex items-center gap-1 text-[10px] font-bold opacity-80'>
                Dari <PrivacyText>{summary.titipan_users || 0}</PrivacyText> pelanggan
              </div>
            </CardContent>
            <Wallet className='absolute top-4 right-4 h-12 w-12 opacity-20' />
          </Card>
            ...
            <div className="flex gap-2">
              ...
              <Button 
                ...
                onClick={() => {
                  ...
                  printBulkThermal(usersToPrint, month, year, settings)
                }}
              >
                Cetak Thermal
              </Button>
              <Button 
                ...
                onClick={() => {
                  const usersToPrint = Array.from(selectedUsersMap.values())
                  printBulkInvoice(usersToPrint, month, year, settings)
                }}
              >
                Cetak Invoice
              </Button>
