...
import { printPaymentCard } from '../utils/print-templates'
            ...
            <Button 
              ...
              onClick={() => {
                if (data?.data && data?.user) {
                  // Wait, how do I pass settings? Usually we fetch it or pass it. We don't have settings here.
                  // I will just pass undefined for settings, it will use defaults like 'WIFIKU NET'.
                  printPaymentCard(data.user, year, data.data, data.summary, undefined);
                }
              }}
            >
              ...
              <table className="w-full text-sm">
                  ...
                  {data?.data?.map((row: any, i: number) => {
                    ...
                    return (
                      <tr key={i} className="hover:bg-muted/50 border-b last:border-0 transition-colors">
                        ...
                        <td className={cn("p-2 border-r text-right", row.kekurangan_kemarin < 0 && "text-emerald-600 dark:text-emerald-400 font-bold")}>{fmtSaldo(row.kekurangan_kemarin)}</td>
                        <td className={cn("p-2 border-r text-right font-bold bg-slate-50 dark:bg-slate-900/50", row.tagihan_bulan_ini < 0 && "text-emerald-600 dark:text-emerald-400")}>{fmtSaldo(row.tagihan_bulan_ini)}</td>
                        <td className={cn(
                ...
                <tfoot>
                  <tr className="bg-slate-100 dark:bg-slate-900 font-bold border-t-2 border-slate-300 dark:border-slate-800">
                    ...
                    <td className={cn("p-3 text-right", (data?.summary?.sisa_akhir || 0) > 0 ? "text-red-600 dark:text-red-400" : "text-emerald-600 dark:text-emerald-400")}>
                      {fmtSaldo(data?.summary?.sisa_akhir || 0)}
                    </td>
                    <td></td>
                  </tr>
                </tfoot>
