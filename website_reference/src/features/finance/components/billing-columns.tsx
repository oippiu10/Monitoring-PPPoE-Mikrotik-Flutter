  ...
  columns.push({
    ...
    cell: ({ row }) => {
      ...
      if (original.status === 'paid') {
        if (original.method === 'titipan') {
          return (
            <div className="flex flex-col items-center gap-1">
              <div
                className="mx-auto flex w-[84px] items-center justify-center gap-1 rounded-full border border-dashed border-pink-400 bg-pink-100/90 px-2.5 py-1 text-[10px] font-extrabold tracking-wider text-pink-700 uppercase shadow-sm shadow-pink-500/10 dark:border-pink-500/50 dark:bg-pink-900/30 dark:text-pink-400 cursor-help"
          ...
          <div className="flex justify-end gap-1.5 pr-4">
            ...
            <DropdownMenu>
              ...
              <DropdownMenuContent align="end" className="w-48 shadow-xl rounded-xl">
                <DropdownMenuItem
                  onClick={() => printThermal(original, ctx.month, ctx.year, ctx.settings)}
                ...
                <DropdownMenuItem
                  onClick={() => printInvoice(original, ctx.month, ctx.year, ctx.settings)}
                  className="cursor-pointer"
                >
                ...
                <DropdownMenuItem
                  onClick={async () => {
                    const ok = await ctx.confirmAction({
                      title: 'Sesuaikan Tagihan',
                      description: `Apakah Anda yakin ingin memperbarui tagihan ${original.username} agar sama dengan harga profil terbarunya?`,
                      confirmText: 'Ya, Sesuaikan',
                      cancelText: 'Batal'
                    })
                    if (ok) ctx.syncInvoice(original.user_id)
                  }}
                  className="cursor-pointer text-indigo-600 focus:bg-indigo-50 focus:text-indigo-700 dark:text-indigo-400 dark:focus:bg-indigo-950/50"
                >
