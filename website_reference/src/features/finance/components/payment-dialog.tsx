...
interface PaymentDialogProps {
  ...
  paidDialog: any
  privacyMode: boolean
  onSave: (payload: { calculatedAmount: number; calculatedNote: string; calculatedMethod?: string; calculatedDate?: string; isDiscount?: boolean }) => void
  isPending: boolean
  fmt: (n: number) => string
}
...
export function PaymentDialog({
  ...
  const formatNum = (val: string) => {
    if (!val) return ''
    const num = parseInt(parseNum(val), 10)
    return isNaN(num) ? '' : num.toLocaleString('id-ID')
  }

  useEffect(() => {
    if (paidDialog) {
      const initAmt = parseInt(String(paidDialog.paid_amount || paidDialog.harga || 0), 10)
          ...
          {isInstallmentMode ? (
            <>
              ...
              <div>
                ...
                <Input
                  ...
                  value={formatNum(installmentAmount)}
                  onChange={(e) => setInstallmentAmount(parseNum(e.target.value))}
            ...
            <>
              <div>
                ...
                <Input
                  type={privacyMode ? 'password' : 'text'}
                  value={formatNum(paidAmount)}
                  onChange={(e) => setPaidAmount(parseNum(e.target.value))}
                  className='mt-1 font-mono rounded-xl'
                  placeholder='0'
                />
                {parseFloat(String(paidDialog.harga || 0)) > 0 && (
                  <div className='flex flex-col gap-2 mt-1.5'>
                    <div className='flex items-center justify-between text-[10px] font-bold text-muted-foreground bg-slate-100 dark:bg-slate-900 px-2 py-1.5 rounded-lg select-none'>
                      <span>
                        Paket:{' '}
                        <span className='font-mono font-extrabold text-foreground'>
                          {fmt(parseFloat(String(paidDialog.harga || 0)))}
                      ...
                      {parseFloat(String(paidAmount || 0)) < parseFloat(String(paidDialog.harga || 0)) && !isDiscount ? (
                    ...
                    {parseFloat(String(paidAmount || 0)) < parseFloat(String(paidDialog.harga || 0)) && parseFloat(String(paidAmount || 0)) > 0 && (
                      <div className="flex items-center space-x-2 bg-indigo-50/50 dark:bg-indigo-950/20 px-2.5 py-2 rounded-lg border border-indigo-100 dark:border-indigo-900/50">
                        <Checkbox 
                          id="isDiscount" 
                          checked={isDiscount} 
                          onCheckedChange={(checked) => setIsDiscount(checked as boolean)}
                          className="h-4 w-4 border-indigo-300 data-[state=checked]:bg-indigo-600"
                        />
                        <label
                          htmlFor="isDiscount"
                          className="text-xs font-bold leading-none cursor-pointer text-indigo-700 dark:text-indigo-400 select-none"
                        >
                          Tandai Lunas Penuh (Berikan Diskon)
                        </label>
                      </div>
                    )}
                  </div>
