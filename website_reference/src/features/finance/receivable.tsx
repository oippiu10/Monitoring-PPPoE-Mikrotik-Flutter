...
export function FinanceReceivable() {
  ...
  const { mutate: mutateOpenIsolate, isPending: openIsolatePending } = openIsolate
  ...
  const columns = useMemo(() => getReceivableColumns({
    ...
    handleOpenIsolate: async (row) => {
      ...
      if (ok) mutateOpenIsolate(row)
    },
    openIsolatePending: openIsolatePending,
    dataLength: data?.data?.length || 0,
    fmt,
    overdueColor
  }), [permissions, selectedRows, toggleSelectRow, toggleSelectAll, setPaymentCardUser, setPaidDialog, confirmAction, openIsolatePending, mutateOpenIsolate, data?.data?.length, month, year, navigate])
