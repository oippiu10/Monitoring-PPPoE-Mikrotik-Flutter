...
function runSQL(mysqli $conn, string $tableName, string $sql) {
    if ($conn->query($sql)) {
...
// ─── 18. Tabel CABLE_COLORS ──────────────────────────────────────────────────────
$results[] = runSQL($conn, 'cable_colors', "
    CREATE TABLE IF NOT EXISTS `cable_colors` (
        `id` INT AUTO_INCREMENT PRIMARY KEY,
        `router_id` VARCHAR(100) NOT NULL,
        `name` VARCHAR(100) NOT NULL,
        `color_code` VARCHAR(20) NOT NULL DEFAULT '#000000',
        `created_at` TIMESTAMP DEFAULT CURRENT_TIMESTAMP
    ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
");
...
$results[] = ensureColumnExists($conn, 'odp', 'gpon_type', "varchar(50) DEFAULT 'GPON'");
$results[] = ensureColumnExists($conn, 'odp', 'rx_power', "varchar(50) DEFAULT NULL");
$results[] = ensureColumnExists($conn, 'odp', 'odp_type_flag', "varchar(50) DEFAULT 'ratio'");
$results[] = ensureColumnExists($conn, 'odp', 'parent_id', "int(11) DEFAULT NULL");
$results[] = ensureColumnExists($conn, 'odp', 'odc_id', "int(11) DEFAULT NULL");
$results[] = ensureColumnExists($conn, 'odp', 'cable_color_id', "int(11) DEFAULT NULL");
$results[] = ensureColumnExists($conn, 'odp', 'core_number', "int(11) DEFAULT NULL");

$results[] = ensureColumnExists($conn, 'odc', 'cable_color_id', "int(11) DEFAULT NULL");
$results[] = ensureColumnExists($conn, 'odc', 'core_number', "int(11) DEFAULT NULL");

$results[] = ensureColumnExists($conn, 'users', 'odp_port', "int(11) DEFAULT NULL");
