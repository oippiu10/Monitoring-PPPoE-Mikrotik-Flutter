...
try {
    ...
    foreach ($updates as $key => $val) {
        ...
        $allowed = ['type', 'splitter_type', 'odc_id', 'parent_id', 'cable_color_id', 'gpon_type', 'rx_power', 'ratio_used', 'ratio_total'];
        if (!in_array($key, $allowed)) continue;

        $set_parts[] = "$key = ?";
        if (is_null($val)) {
            $types .= "s"; // bind null as string, akan jadi NULL di MySQL
            $params[] = null;
        } elseif (is_int($val)) {
            $types .= "i";
            $params[] = $val;
    ...
    if (empty($set_parts)) {
        echo json_encode(["success" => false, "error" => "Tidak ada kolom valid untuk diupdate"]);
        exit();
    }
