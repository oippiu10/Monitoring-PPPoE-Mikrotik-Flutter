...
function table_has_column(mysqli $conn, string $table, string $column) {
...
function ensure_network_lines_table(mysqli $conn) {
    ...
    // Auto-heal: Jika kolom source_id atau target_id ada, pastikan mereka memiliki default value agar tidak error saat INSERT
    if (table_has_column($conn, 'network_lines', 'source_id')) {
        @$conn->query("ALTER TABLE `network_lines` MODIFY `source_id` varchar(100) NOT NULL DEFAULT ''");
    }
    if (table_has_column($conn, 'network_lines', 'target_id')) {
        @$conn->query("ALTER TABLE `network_lines` MODIFY `target_id` varchar(100) NOT NULL DEFAULT ''");
    }
}
...
try {
    ...
    switch ($method) {
        ...
        case 'POST':
            ...
            $stmt = $conn->prepare('INSERT INTO network_lines (router_id, name, type, path, color, source_id, target_id) VALUES (?, ?, ?, ?, ?, ?, ?)');
            if (!$stmt) throw new Exception($conn->error);
            $stmt->bind_param('sssssss', $routerId, $name, $type, $path, $color, $sourceId, $targetId);
            $stmt->execute();
            $stmt->close();
            log_admin_activity($conn, 'network_line_create', "Tambah kabel/jalur: {$name}", (int)($_SESSION['admin_id'] ?? 0));
        ...
        case 'DELETE':
            ...
            $stmt->execute();
            $stmt->close();
            log_admin_activity($conn, 'network_line_delete', "Hapus kabel/jalur ID: {$id}", (int)($_SESSION['admin_id'] ?? 0));
    ...
    @file_put_contents(__DIR__ . '/../scratch/debug_log.txt', "ERROR: " . $e->getMessage() . "\n" . "METHOD: " . $_SERVER['REQUEST_METHOD'] . "\n" . "INPUT: " . file_get_contents('php://input') . "\n\n", FILE_APPEND);
