...
try {
    ...
    foreach ($updates as $key => $val) {
        // Handle unassigning odp_id (convert empty, 'none', or 0 to null)
        if ($key === 'odp_id' && ($val === '' || $val === 'none' || $val === 0 || $val === '0')) {
            $val = null;
        }

        $set_parts[] = "$key = ?";
        if ($val === null) {
            $types .= "s"; // mysqli bind_param accepts 's' for null values
        } elseif (is_int($val)) {
            $types .= "i";
    ...
    log_admin_activity($conn, 'customer_bulk_update', "Update massal {$success_count} pelanggan ({$keys})", (int)($_SESSION['admin_id'] ?? 0));
