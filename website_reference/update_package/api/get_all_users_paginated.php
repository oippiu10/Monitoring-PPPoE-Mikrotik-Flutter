...
// Sorting logic
$validSorts = ['id', 'username', 'tanggal_dibuat', 'redaman', 'wa', 'tanggal_tagihan', 'profile', 'odp_id', 'odp_name', 'status', 'tipe_langganan'];
...
try {
    ...
    if ($odp !== '') {
        if ($odp === 'none' || $odp === '0') {
            $where[] = '(u.odp_id IS NULL OR u.odp_id = 0)';
        } else {
            $where[] = 'u.odp_id = ?';
            $params[] = $odp;
            $types .= 's';
    ...
    $sqlSortBy = 'u.username';
    if ($sortBy === 'id') $sqlSortBy = 'u.id';
    elseif ($sortBy === 'username') $sqlSortBy = 'u.username';
    elseif ($sortBy === 'tanggal_dibuat') $sqlSortBy = 'u.tanggal_dibuat';
    elseif ($sortBy === 'redaman') $sqlSortBy = 'u.redaman';
    elseif ($sortBy === 'wa') $sqlSortBy = 'u.wa';
    elseif ($sortBy === 'tanggal_tagihan') $sqlSortBy = 'u.tanggal_tagihan';
    elseif ($sortBy === 'profile') $sqlSortBy = 'u.profile';
    elseif ($sortBy === 'odp_id' || $sortBy === 'odp_name') $sqlSortBy = 'o.name';
    elseif ($sortBy === 'tipe_langganan') $sqlSortBy = 'u.tipe_langganan';

    $dataSQL = "SELECT
                    u.id, u.username, u.password, u.profile, u.wa, u.maps, u.lat, u.lng, u.foto,
                    DATE_FORMAT(u.tanggal_dibuat, '%Y-%m-%d %H:%i:%s') as tanggal_dibuat, u.odp_id, u.odp_port, u.created_at, u.updated_at, u.alamat, u.redaman, u.tanggal_tagihan, u.tipe_langganan,
                    o.name as odp_name
                 FROM users u
                 LEFT JOIN odp o ON u.odp_id = o.id
                 $whereSQL
                 ORDER BY $sqlSortBy $sortOrder";
    ...
    if (empty($profiles)) {
        ...
        $stmtPDB = $conn->prepare("SELECT profile_name FROM ppp_profile_pricing WHERE router_id = ? ORDER BY profile_name");
    ...
    $odps = [];
    $stmtO = $conn->prepare("SELECT id, name, capacity, type, splitter_type, ratio_total, gpon_type, rx_power, odp_type_flag FROM odp WHERE router_id = ? ORDER BY name ASC");
