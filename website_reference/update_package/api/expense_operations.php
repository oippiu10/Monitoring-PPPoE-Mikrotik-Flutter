...
require_admin_role(['admin', 'administrator', 'finance'], 'Akses ditolak. Hanya admin/finance yang boleh mengelola pengeluaran.');
...
switch ($action) {
    ...
    case 'add':
        ...
        if ($stmt->execute()) {
            log_admin_activity($conn, 'expense_add', "Menambahkan pengeluaran kategori {$data['category']} sebesar " . floatval($data['amount']), (int)($_SESSION['admin_id'] ?? 0));
    ...
    case 'edit':
        ...
        if ($stmt->execute()) {
            log_admin_activity($conn, 'expense_edit', "Mengubah pengeluaran ID {$data['id']} kategori {$data['category']} menjadi " . floatval($data['amount']), (int)($_SESSION['admin_id'] ?? 0));
    ...
    case 'delete':
        ...
        if ($stmt->execute()) {
            log_admin_activity($conn, 'expense_delete', "Menghapus pengeluaran ID {$id}", (int)($_SESSION['admin_id'] ?? 0));
